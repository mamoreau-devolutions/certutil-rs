//! Compatibility checks between **`certutil.exe -verify`** and **`certutil-rs -verify`** on the **same** leaf file.
//!
...
/// Host used for TLS prefetch and for `-sslpolicy` / `--ssl-dns-name` compatibility checks.
...
#[test]
#[ignore = "live network + certutil.exe on PATH: compatibility check default -verify"]
...
#[test]
#[ignore = "live network + certutil.exe on PATH: compatibility check -verify -urlfetch -t"]
...
#[test]
#[ignore = "live network + certutil.exe on PATH: compatibility check SSL hostname policy"]
