...
/// Read-only certificate diagnostics (no store changes) built around Windows certificate APIs and certutil-compatible workflows.
#[derive(Debug, Parser)]
...
pub enum Command {
    ...
    /// TLS utilities: fetch peer leaf certificate for export.
    Tls {
        #[command(subcommand)]
        action: TlsAction,
    },
...
#[derive(Debug, Subcommand)]
pub enum TlsAction {
    /// Connect with TLS, save the server leaf certificate (PEM or DER) for later verification or inspection.
    Fetch {
