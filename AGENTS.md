...
Build a **Windows-only** Rust tool that tracks **`certutil.exe`** behavior as closely as practical for **certificate validation diagnostics**: incomplete chains, revocation (CRL/OCSP/CDP), policy errors, and readable inspection of cert material. Prefer the same **Win32 CryptoAPI / certificate chain APIs** exposed through the **`windows`** crate—not cross-platform crypto stacks for core validation logic.
...
- Local copies of Microsoft binaries may be kept under **`.local-reference/`** for manual compatibility checks; keep them ignored by git.
- Do not commit **`certutil.exe`** / **`certcli.dll`** or any generated analysis databases.
...
- **Reference:** Windows CryptoAPI behavior and **`certutil.exe`** compatibility checks.
