...
Windows-only Rust tooling for **read-only** certificate diagnostics on top of the Windows CryptoAPI and certificate chain APIs. The tool focuses on certificate chains, revocation (CRL/CDP/OCSP) troubleshooting, and readable inspection of certificate material without modifying certificate stores or other system state.
...
**Compatibility check against `certutil.exe -verify`** (prefetches the leaf via **`tls fetch`**, then runs both tools on the same `.cer`):
...
`-verify` loads the encoded cert, calls **`CertGetCertificateChain`** (with revocation checking on the chain, excluding the root) and **`CertVerifyCertificateChainPolicy`** with **`CERT_CHAIN_POLICY_BASE`**, then prints trust status and chain elements. Options **`--urlfetch`** and **`-t`** follow the Windows URL retrieval behavior used during chain building. **`--ssl-dns-name`** runs **`CERT_CHAIN_POLICY_SSL`** (server profile); **`--ssl-client-dns-name`** runs the SSL policy with the **client** authentication type. Additional optional passes cover common Windows policy checks: **`--policy-authenticode`**, **`--policy-authenticode-ts`**, **`--policy-basic-constraints`**, **`--policy-nt-auth`**.
...
Compatibility check with **`certutil.exe`** on the saved file:
...
- [README.md](README.md) documents build, usage, and live test entry points.
